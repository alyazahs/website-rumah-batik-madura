...
<div class="container mx-auto px-4 py-8">
    <div class="bg-white rounded-2xl shadow-lg border border-gray-200 flex flex-col md:flex-row gap-10 p-8">
        <!-- Gambar Produk -->
        <div class="md:w-1/2 flex justify-center items-center">
            <img src="{{ asset('storage/' . $product->path) }}" 
                 alt="{{ $product->nameProduct }}" 
                 class="rounded-2xl max-h-[400px] w-full object-cover shadow-lg">
        </div>

        <!-- Informasi Produk -->
        <div class="md:w-1/2 flex flex-col justify-between">
            <div>
                <h1 class="text-4xl font-extrabold text-gray-900 mb-3">{{ $product->nameProduct }}</h1>
                <p class="text-indigo-600 text-3xl font-semibold mb-5">Rp{{ number_format($product->price, 0, ',', '.') }}</p>
            ...
            <div class="mt-8">
                <a href="https://wa.me/6285927457129?text=Halo,%20saya%20tertarik%20dengan%20produk%20{{ urlencode($product->nameProduct) }}"
                   target="_blank"
                   class="flex items-center justify-center gap-3 bg-green-600 hover:bg-green-700 text-white font-bold py-4 rounded-xl shadow-lg transition duration-300">
